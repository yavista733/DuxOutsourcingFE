...
import { Plus, X, Users, ShieldCheck, UserCheck, Pencil, ToggleLeft, ToggleRight } from 'lucide-react';
...
export default function Configuracion() {
  ...
  const handleToggleEstado = async (usuario) => {
    const confirmMsg = usuario.activo
      ? `¿Desactivar a ${usuario.username}? No podrá iniciar sesión.`
      : `¿Activar a ${usuario.username}?`;
    ...
    <Layout>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {usuarios.length === 0 ? (
                  ...
                  usuarios.map((u) => (
                    <tr key={u.id} className={`border-b border-slate-50 transition-colors ${!u.activo ? 'opacity-50' : 'hover:bg-slate-50'}`}>
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-2">
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center ${u.activo ? 'bg-emerald-100' : 'bg-slate-100'}`}>
                            <span className={`text-xs font-bold ${u.activo ? 'text-emerald-700' : 'text-slate-400'}`}>
                      ...
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          {u.rol?.nombre !== 'ADMIN' && (
                            ...
                            >
                                {u.activo
                                ? <><ToggleRight size={12} /> Desactivar</>
                                : <><ToggleLeft size={12} /> Activar</>
                                }
                            </button>
                            )}
                        </div>
      ...
      {modalCrear && (
        <ModalCrearUsuario
          ...
          onGuardado={() => { setModalCrear(false); cargarDatos(); }}
        />
      )}
      ...
      {usuarioEditar && (
        <ModalEditarUsuario
          ...
          onGuardado={() => { setUsuarioEditar(null); cargarDatos(); }}
        />
      )}
    </Layout>
  );
...
// ── Modal Crear Usuario ──────────────────────────────────────
function ModalCrearUsuario({ areas, onCerrar, onGuardado }) {
  const [form, setForm] = useState({
    username: '', password: '', nombreCompleto: '', email: '', rolId: '', areaId: '',
  });
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="text" name="username" value={form.username} onChange={handleChange} placeholder="Ej: jperez"
          ...
          <div>
            ...
            <input type="text" name="nombreCompleto" value={form.nombreCompleto} onChange={handleChange} placeholder="Ej: Juan Pérez"
              className="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-slate-700 text-sm focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500" />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1.5">Email</label>
            <input type="email" name="email" value={form.email} onChange={handleChange} placeholder="Ej: jperez@dux.com"
              className="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-slate-700 text-sm focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
          ...
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={onCerrar} className="flex-1 py-2.5 rounded-xl border border-slate-200 text-slate-600 text-sm font-medium hover:bg-slate-50 transition-colors">Cancelar</button>
            <button type="submit" disabled={guardando} className="flex-1 py-2.5 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-white text-sm font-medium transition-colors disabled:opacity-50">
...
function ModalEditarUsuario({ usuario, areas, onCerrar, onGuardado }) {
  const [form, setForm] = useState({
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.nombreCompleto || !form.email || !form.rolId || !form.areaId) {
      setError('Todos los campos son obligatorios.');
      return;
    }
    setGuardando(true);
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">@{usuario.username}</p>
          </div>
          <button onClick={onCerrar} className="text-slate-400 hover:text-slate-600"><X size={20} /></button>
        </div>
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          {/* Aviso sin campo contraseña */}
          <div className="bg-slate-50 border border-slate-200 text-slate-500 text-xs px-4 py-3 rounded-xl">
